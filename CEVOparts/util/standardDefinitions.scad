...
module theExtrusion(extrusion, leng) {linear_extrude(nnv(leng, extrusion[length])) profile(extrusion[exmainsq], extrusion[excentersq], extrusion[excornersq],
extrusion[exflangethick], extrusion[exflangelen]);}
...
module screwAllHole(screw, leng, depth = 1, spacing = 20) {
    screwHole(screw, nnv(leng, screw[length])+depth*screw[headHeight]);
